...
const paymentCreate =async(req,res)=>{
    try {
        const data=req.body;
        const saveData=await Payment.create(data);
...
const paymentFetch=async(req,res)=>{
  try {
      ...
      const getUser=await Payment.find()
      if(!getUser){
...
const paymentUpdate=async(req,res)=>{
  try {
      ...
      const updateUser=await Payment.updateOne({_id:id},{$set:data})
...
const paymentDeleted=async(req,res)=>{
    try {
        const data=req.body;
        const deleteData=await Payment.deleteOne(data);
...
const paymentSearched=async(req,res)=>{
    try {
        const data=req.body;
        const searchData=await Payment.find();
...
module.exports={paymentCreate,paymentFetch,paymentUpdate,paymentDeleted,paymentSearched}